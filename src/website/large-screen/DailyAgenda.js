...
export const FIND_CLASS_PERIOD = gql`
  query findClassPeriodForDailyAgenda(
    $assignedDate: Date
    $period: periodName
  ) {
...
const DailyAgenda = ({ match }) => {
  ...
  const [oldLessonDateForDisplay, setOldLessonDateForDisplay] = useState(
    todaysDate
  )
        ...
        >
          ...
          >
            ...
            >
              ...
              {match.params.courseName.substring(2, 3)}-
            ...
            {oldLessonToggle && (
              <>
                <input
                  ...
                  type='date'
                  value={oldLessonDate}
                  onChange={(e) => setOldLessonDate(e.target.value)}
                ></input>
                <Link
                  style={{
                    marginRight: '1%',
