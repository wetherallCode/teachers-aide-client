...
const OpenEndedAssignmentDownloader = ({
  ...
  const [assignmentTypeFilterValue, setAssignmentTypeFilterValue] = useState(
    'OEQ'
  )
  const [classPeriodAssignments, setClassPeriodAssignments] = useState([])
  const [assignmentValue, setAssignmentValue] = useState('')
  const [markingPeriodValue, setMarkingPeriodValue] = useState(
    currentMarkingPeriod
  )
  ...
  useEffect(() => {
    homework.forEach((element) =>
      element.forEach((assignment) =>
        setClassPeriodAssignments((list) => [...list, assignment])
      )
    )
