...
const AssignmentModifierDisplay = ({
  ...
  const [updateAssignmentCheckPoint, setUpdateAssignmentCheckPoint] = useState(
    false
  )
  ...
  return (
    ...
    >
        ...
        {!readingSectionToggle ? (
          ...
          <input
            ...
            onChange={(e) =>
              setUpdatedAssignment({
                ...updatedAssignment,
                readingSections: e.target.value,
              })
            }
          />
        )}
      ...
      >
        ...
        {!dueDateToggle ? (
          <div onClick={() => setDueDateToggle(!dueDateToggle)}>
            {assignment.dueDate}
          </div>
        ) : (
          <input
      ...
      >
        ...
        {!markingPeriodToggle ? (
          <div onClick={() => setMarkingPeriodToggle(true)}>
            {updatedAssignment.markingPeriod}
          </div>
        ) : (
          <select
            style={{ width: '10rem', fontSize: '90%', color: 'var(--blue)' }}
            value={updatedAssignment.markingPeriod}
            onChange={(e) =>
              setUpdatedAssignment({
                ...updatedAssignment,
                markingPeriod: e.target.value,
              })
            }
          >
